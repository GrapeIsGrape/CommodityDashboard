...
_Last reconciled: 2026-06-16 (backlog cleared: #8 schema_version on /health + #12 curve realized-front anchoring both shipped — QA✓ UAT✓. Loop PAUSED by user after #12. Next on resume: Phase 4 dashboard or the deferred scheduler.)_
...
Open backlog: ~~**#8** (`schema_version` on `/health`)~~ **DONE** (read-only Alembic revision, `null` pre-migration) · ~~**#12** (curve-shape deferred-gap anchoring)~~ **DONE** (config-driven realized-front expiry rules; slope magnitude correctly scaled near rolls). **Backlog clear** — next work is Phase 4 dashboard or the deferred scheduler.
