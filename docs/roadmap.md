...
_Last reconciled: 2026-06-18 (Phase 4 IN PROGRESS: #13 Panel D, #14 Panel C, #15 Panel A, #16 Panel A polish, #18 Panel B all shipped — SecAudit✓ QA✓ UAT✓. #18 UAT found 1 contained issue: the weekly cold-start percentile threshold was 52 but the percentile counts seasonal comparables (~3/yr), so the tight/loose column was effectively perma-accruing and "/52" misled; amended in-ticket to PANEL_B_MIN_HISTORY_WEEKLY=9 (≈3 prior seasons) with an honest M/N denominator. Follow-ups filed: #19 (Cushing tank-bottom proximity). #20 prices-ETL → `prices` (TLT/VTI/QQQ, raw close + total-return adj_close, migration 0003) shipped — SecAudit✓ QA✓ (incl. independent migration upgrade/downgrade run) UAT✓ — UNBLOCKS the macro-context sub-panel. Next: Phase 4 macro sub-panel render (now unblocked) + sentiment placeholder, or the deferred scheduler.)_
...
Open backlog: ~~**#8**~~ DONE · ~~**#12**~~ DONE · ~~**#13** (foundation + Panel D)~~ **DONE** · ~~**#14** (Panel C)~~ **DONE** · ~~**#15** (Panel A)~~ **DONE** · ~~**#16** (Panel A polish)~~ **DONE** · ~~**#18** (Panel B fundamentals/inventory)~~ **DONE**. ~~**#20** (prices-ETL writer + migration 0003)~~ **DONE**. Open: **#19** (Panel B Cushing tank-bottom proximity — nice-to-have follow-up, later cycle). Next: the **macro-context sub-panel render** (TLT/VTI/QQQ → `prices`, now unblocked by #20 — renders `adj_close` labeled "total-return adjusted", NYSE-calendar daily staleness) or the deferred scheduler.
