...
**Stop after each phase for review.** Current position: **Phase 3 in progress — IV → `iv_metrics` done (#9), vol indices GVZ/OVX → `iv_metrics` done (#10); curve shape still to do (VIX intentionally excluded — sourced from FRED `VIXCLS` → `macro_metrics`). Phase 2 free-data ETL complete (FRED #3, EIA #4, USDA #6, CFTC #7); scheduler wiring still deferred.**
...
- **Repository structure:**
  ...
  - `config/symbols.yaml` — v1 commodity universe + commodity→optionable-ETF-proxy mapping + macro-context tickers + `volatility_indices` (GVZ/OVX ingest map with per-entry `ticker`/`symbol`/`ingest` + `backfill_days`; VIX excluded via `ingest: false`) — the canonical symbol list
  ...
  - `common/config.py` — shared: `get_database_url()` (env→SQLAlchemy URL) + `load_symbols()` + `load_fred_series()` + `load_eia_series()` + `load_usda_series()` + `load_cftc_markets()`
  - `dashboard/` — FastAPI app (`main.py`: `/` boot page, `/health` Postgres check), `Dockerfile`, `requirements.txt`
  - `etl/` — `run.py` (entrypoint: applies migrations then idles — no scheduler yet), `sources/` (one module per source: `fred.py` Phase 2 → Panel A, `eia.py` + `usda.py` Phase 2 → Panel B, `cftc.py` Phase 2 → Panel C, `iv.py` + `vol_indices.py` Phase 3 → Panel D (`iv.py` option-chain IV behind a swappable `get_iv()`; `vol_indices.py` GVZ/OVX index levels behind a swappable `IndexHistoryProvider`)), `Dockerfile`, `requirements.txt` (adds `requests`, `yfinance`)
  ...
  - `tests/` — pytest (`test_config.py`, `test_migrations.py`, `test_fred.py`, `test_eia.py`, `test_usda.py`, `test_cftc.py`, `test_iv.py`, `test_vol_indices.py` — live-Postgres-or-skip; external APIs/providers mocked)
