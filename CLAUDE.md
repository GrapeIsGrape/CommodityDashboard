...
- **Repository structure:**
  ...
  - `dashboard/` — FastAPI app (`main.py`: `/` boot page, `/health` read-only Postgres check that also returns `schema_version` = current Alembic revision, `null` on a pre-migration DB — #8, `GET /panel/d` Panel D — #13), `templates/` (Jinja2: `base.html`, reusable `_panel.html` shell, `panel_d.html`), `panels/` (`panel_d.py` — read-only Panel D view model + pure logic: conjunctive highlight, accruing `— (N/20)`, staleness vs last expected session, GVZ/OVX strip, DB-error isolation), `Dockerfile`, `requirements.txt` (adds `Jinja2`)
  ...
  - `tests/` — pytest (`test_config.py`, `test_migrations.py`, `test_fred.py`, `test_eia.py`, `test_usda.py`, `test_cftc.py`, `test_iv.py`, `test_vol_indices.py`, `test_curve_shape.py`, `test_health.py`, `test_panel_d.py` — live-Postgres-or-skip; external APIs/providers mocked)
