...
**Stop after each phase for review.** Current position: **Phase 2 COMPLETE — all four free-data ETL sources landed (FRED #3, EIA #4, USDA #6, CFTC #7); scheduler wiring still deferred. Next: Phase 3 (Volatility & positioning ETL).**
...
- **Repository structure:**
  ...
  - `etl/` — `run.py` (entrypoint: applies migrations then idles — no scheduler yet), `sources/` (one module per source: `fred.py` Phase 2 → Panel A, `eia.py` + `usda.py` Phase 2 → Panel B, `cftc.py` Phase 2 → Panel C), `Dockerfile`, `requirements.txt` (adds `requests`)
  ...
  - `tests/` — pytest (`test_config.py`, `test_migrations.py`, `test_fred.py`, `test_eia.py`, `test_usda.py`, `test_cftc.py` — live-Postgres-or-skip; external APIs mocked)
